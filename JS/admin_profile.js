...
function renderPost(data, postId) {
  ...
  if (data.repostOf) {
    ...
    bodyHtml = `
      ...
      <div class="post-body clamped" id="body-${postId}">${bodyHTML}</div>
...
window.togglePinPost = async function (postId, currentlyPinned) {
  try {
    if (!currentlyPinned) {
      ...
      await batch.commit();
      window.showToast("Post pinned to homepage!", "success");
    } else {
      // Just unpin
      await updateDoc(doc(db, "announcements", postId), { pinned: false });
      window.showToast("Post unpinned.", "success");
    }
  } catch (err) {
    console.error("Pin error:", err);
    window.showToast("Failed to update pin status.", "error");
  }
};
...
window.viewReposts = async function (postId, e) {
  ...
  try {
    ...
    snap.forEach(docSnap => {
      ...
      let time = 'Just now';
      if (data.createdAt) {
        const d = (data.createdAt && data.createdAt.toDate) ? data.createdAt.toDate() : new Date(data.createdAt);
        const dateStr = d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        const timeStr = d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
